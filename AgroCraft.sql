...
CREATE TABLE `farmer_orders` (
  ...
  CONSTRAINT `farmer_orders_ibfk_1` FOREIGN KEY (`farmer_id`) REFERENCES `farmerregistration` (`farmer_id`),
  CONSTRAINT `farmer_orders_ibfk_2` FOREIGN KEY (`buyer_id`) REFERENCES `buyerregistration` (`buyer_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Table structure for table `farmer_orders`
--
INSERT INTO farmer_orders (buyer_id, farmer_id, product_name, quantity, status, timestamp) 
VALUES (1, 2, 'Tomatoes', 5, 'Placed', NOW());
